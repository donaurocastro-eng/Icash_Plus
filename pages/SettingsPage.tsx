...
const SettingsPage: React.FC = () => {
  ...
  const checkSchema = async (urlOverride?: string) => {
    ...
    try {
      ...
      // 2. Check for next_payment_date column (Payment Control Check)
      const contractColCheck = await db.query(
        "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'contracts' AND column_name = 'next_payment_date';"
      );
  ...
  const handlePatchContracts = async () => {
    ...
    try {
      ...
      addLog("2. Agregando columna apartment_code...");
      ...
      addLog("✅ Restricciones actualizadas.");
      
      alert("¡Reparación exitosa! Columnas agregadas.");
  ...
  const handleInitializeStepByStep = async () => {
    ...
    try {
        ...
        { name: "Reparar: Contratos (Unidad)", sql: `ALTER TABLE public.contracts ADD COLUMN IF NOT EXISTS apartment_code text;` },
        { name: "Reparar: Contratos (Próximo Pago)", sql: `ALTER TABLE public.contracts ADD COLUMN IF NOT EXISTS next_payment_date date; UPDATE contracts SET next_payment_date = start_date WHERE next_payment_date IS NULL;` },
        {
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6">
          ...
          <p className="text-slate-500 text-sm mb-6">
            Si ves errores de "columna no existe" (como next_payment_date), presiona el botón amarillo de Parchear.
          </p>
          ...
          <div className="space-y-3">
            ...
            <button 
                ...
                className={`w-full flex items-center justify-center space-x-2 px-4 py-4 text-white rounded-lg font-bold shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed bg-indigo-600 hover:bg-indigo-700`}
            >
                {initLoading ? (
                ...
                <span className="text-lg">Inicializar / Reparar Tablas</span>
            </button>
